...
describe('selectBestXI (4-4-2)', () => {
  it('returns 11 slots', () => {
    expect(selectBestXI(squad442())).toHaveLength(11);
  });

  it('assigns the correct slot labels', () => {
    const slots = selectBestXI(squad442()).map(s => s.slot);
    expect(slots).toEqual(['GK', 'RB', 'CB1', 'CB2', 'LB', 'RM', 'CM1', 'CM2', 'LM', 'ST1', 'ST2']);
  });

  it('fills both striker slots', () => {
    const xi = selectBestXI(squad442());
...
describe('selectBestXI (5-3-2)', () => {
  it('returns 11 slots', () => {
    expect(selectBestXI(squad532())).toHaveLength(11);
  });

  it('assigns the correct slot labels', () => {
    const slots = selectBestXI(squad532()).map(s => s.slot);
    expect(slots).toEqual(['GK', 'RWB', 'CB1', 'CB2', 'CB3', 'LWB', 'CM1', 'CM2', 'CM3', 'ST1', 'ST2']);
  });

  it('fills all three CB slots', () => {
    const xi = selectBestXI(squad532());
